...
public class ClientCaller extends Agent implements SipListener {
	...
	private Log logger = (Log) LogFactory.getLog(ClientCaller.class);
	...
	class MyTimerTask extends TimerTask {
		ClientCaller myListener;
		public MyTimerTask (ClientCaller myListener){
...
//				//myDataSource = Activator.ds;
				...
				destination = ConfigCallerFile.destination;				
				dialTimes = 30; 				
				useQueue = "no"; 				
				mySipAlias = ConfigCallerFile.myServerDomain; 
				myAudioPort= this.getAvailablePort(); //ConfigCallerFile.myAudioPort;; 			
				myProxyIP = ConfigCallerFile.myProxyIp;
				myProxyPort = ConfigCallerFile.myProxyPort;
				mySipFactory = SipFactory.getInstance();
				mySipFactory.setPathName("gov.nist");
				
				myName = ConfigCallerFile.myUserID;
				myUserID = ConfigCallerFile.myUserID + "@" + ConfigCallerFile.myServerDomain;
				myPassword = ConfigCallerFile.myPassword;
				myServer = ConfigCallerFile.myServerDomain;
				myPort = ConfigCallerFile.myPort;
				myIP = ConfigCallerFile.myIp;
	...
	public void processRequest(RequestEvent requestReceivedEvent) {
		///System.out.println("=> BUNDLE: br.ufes.inf.ngn.televoto.client.logic | CLASS: ClientAgent | METOD: processRequest ");//By Ju
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		///System.out.println("=> BUNDLE: br.ufes.inf.ngn.televoto.client.logic | CLASS: ClientAgent | METOD: processResponse ");//By Ju
		try{
			...
			case WAIT_FINAL:
				...
				if (myStatusCode<200) {
					...
					if( status == ESTABLISHED ) {
						...
						// Estou usando o Ip do pc pois o macbook interage bem com a rede vmnet2
						
						//SendMedia(answerInfo.IpAddress);
						
						SendMedia("192.168.2.107");
	...
	public int getAvailablePort() {
		...
		try {
			...
			URL url = new URL("http://localhost:3000/ports/getAvailable");
			...
			while ((output = br.readLine()) != null) {
				...
				port = Integer.parseInt(output.split(":")[1].split("}")[0]);
			}
	...
	public void SendMedia(String IP) {
		
		// Temporario: Somente usado para carregar o audio do que simula uma ligacao
		ConfigCalleeFile configCalleeFile = new ConfigCalleeFile();

		SendStream ss;
		
		at = new AVTransmit2(configCalleeFile.ds, IP, this.myAudioPort, new AudioFormat(AudioFormat.GSM_RTP, 8000, 8, 1), myName);
